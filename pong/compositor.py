	...
	def produce(self):
		...
		while True: 
			...
			for i in xrange(len(self.geometries)):
				...
				# SAMPLE blanking pts between geometries
			...
			###
			###  FRAME END -- blank again to first object. 
			###

			xDest = self.geometries[0].points[0][0]
			yDest = self.geometries[0].points[0][1]
			xDiff = xDest - self.geometries[-1].points[-1][0]
			yDiff = yDest - self.geometries[-1].points[-1][1]

			# SAMPLE blanking pts between objects
			# TODO: In the future, a heuristic for this to make longer
			# paths sample more blanking
			BLANK_POINTS = 100.0
			for i in xrange(1, int(BLANK_POINTS)):
